...
**假设场景:**
开发任务已经分配给开发人员A，A需要在上版本`v1.0.0`开发新功能`usermanage(用户管理)`，开发完成提交项目管理者B进行`代码审核`，审核完成后进行测试，组长B需要为这一次的修复和新功能创建新版本，A需要对上一版本`v1.0.1`中内容进行修改项目。

git库：git@192.168.8.37:homingh/git-manage-tools.git
开发新模块：userManage
是否有遗留bug：是
是否发布新版本：是
